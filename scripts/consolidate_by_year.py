...
from asset_manager.datastore import (
    _daily_object_names,
    _yearly_object_names,
    DAILY_SUMMARY_NAME_REGEX,
    YEARLY_SUMMARY_NAME_REGEX,
    _read_df,
)
...
if existing_object is not None:
    print(
        f"Found existing object for year {year}: '{existing_object}. Will append to it."
    )
...
for daily_object_name in _daily_object_names():
    ...
    if found_year == year:
        print(
            f"Found object '{daily_object_name}'; adding its contents to yearly DataFrame"
        )
...
print(
    "I haven't actually bothered with automating the deletion of the consolidated objects, sorry. Here are their names:"
)
