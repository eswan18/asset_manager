...
from asset_manager.clean import convert_dollar_cols_to_float
...
def test_convert_dollar_cols_to_float():
    ...
    assert_series_equal(
        test_df["definitelynotdollars"], result_df["definitelynotdollars"]
    )
    ...
    assert_series_equal(
        expected_some_dollars, result_df["somedollars"], check_names=False
    )
    expected_also_dollars = pd.Series([0, 55_555.55], dtype=float)
    assert_series_equal(
        expected_also_dollars, result_df["alsodollars"], check_names=False
    )
